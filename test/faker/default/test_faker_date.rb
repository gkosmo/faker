...
class TestFakerDate < Test::Unit::TestCase
  ...
  def test_between
    ...
    deterministically_verify -> { @tester.between(from: from, to: to) } do |random_date|
  ...
  def test_between_except
    ...
    deterministically_verify -> { @tester.between_except(from: from, to: to, excepted: excepted) } do |random_date|
  ...
  def test_forward
    ...
    deterministically_verify -> { @tester.forward(days: 5) } do |random_date|
  ...
  def test_forward_with_string_parameter
    ...
    deterministically_verify -> { @tester.forward(from: from, days: 5) } do |random_date|
  ...
  def test_backward
    ...
    deterministically_verify -> { @tester.backward(days: 5) } do |random_date|
      assert random_date < today, "Expected < \"#{today}\", but got #{random_date}"
    end
  end
  ...
  def test_birthday
    ...
    deterministically_verify -> { @tester.birthday(min_age: min, max_age: max) } do |birthday|
  ...
  def test_default_birthday
    ...
    deterministically_verify -> { @tester.birthday } do |birthday|
      assert birthday >= birthdate_min, "Expect >= \"#{birthdate_min}\", but got #{birthday}"
      assert birthday < birthdate_max, "Expect < \"#{birthdate_max}\", but got #{birthday}"
    end
  end
