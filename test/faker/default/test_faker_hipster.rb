...
class TestFakerHipster < Test::Unit::TestCase
  ...
  def test_word_without_spaces
    ...
    deterministically_verify -> { @tester.word } do |_word|
      refute_match(/\s/, @tester.word)
    end
  end
  ...
  def test_sentence_with_open_compounds_allowed
    deterministically_verify -> { @tester.sentence(word_count: 5, random_words_to_add: 0, open_compounds_allowed: true) } do |sentence|
  ...
  def test_sentence_without_open_compounds_allowed
    deterministically_verify -> { @tester.sentence(word_count: 5, random_words_to_add: 0, open_compounds_allowed: false) } do |sentence|
