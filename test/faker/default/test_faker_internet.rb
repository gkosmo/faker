...
class TestFakerInternet < Test::Unit::TestCase
  ...
  def test_private_ip_v4_address
    ...
    deterministically_verify -> { @tester.private_ip_v4_address } do |address|
  ...
  def test_public_ip_v4_address
    ...
    deterministically_verify -> { @tester.public_ip_v4_address } do |address|
  ...
  def test_ip_v4_cidr
    ...
    deterministically_verify -> { @tester.ip_v4_cidr.split('/').last.to_i } do |cidr|
      assert((1..32).cover?(cidr))
    end
  end
  ...
  def test_mac_address
    ...
    deterministically_verify -> { @tester.mac_address.split(':').map { |d| d.to_i(16) }.max } do |max_d|
      assert max_d <= 255
    end
  ...
  def test_ip_v6_address
    ...
    deterministically_verify -> { @tester.ip_v6_address.split(':').map { |h| "0x#{h}".hex }.max } do |max_h|
      assert max_h <= 65_535
    end
  end
  ...
  def test_ip_v6_cidr
    ...
    deterministically_verify -> { @tester.ip_v6_cidr.split('/').last.to_i } do |cidr|
      assert((1..128).cover?(cidr))
    end
  end
