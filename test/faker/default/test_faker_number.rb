...
class TestFakerNumber < Test::Unit::TestCase
  ...
  def test_between
    deterministically_verify -> { @tester.between(from: -50, to: 50) } do |random_number|
  ...
  def test_within
    deterministically_verify -> { @tester.within(range: -50..50) } do |random_number|
      assert random_number >= -50, "Expected >= -50, but got #{random_number}"
      assert random_number <= 50, "Expected <= 50, but got #{random_number}"
    end
  end
  ...
  def test_positive
    deterministically_verify -> { @tester.positive(from: 1, to: 100) } do |random_number|
  ...
  def test_negative
    deterministically_verify -> { @tester.negative(from: -1, to: -100) } do |random_number|
      assert random_number <= -1,   "Expected <= -1, but got #{random_number}"
      assert random_number >= -100, "Expected >= -100, but got #{random_number}"
    end
  end
