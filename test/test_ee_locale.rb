...
class TestEeLocale < Test::Unit::TestCase
  ...
  def test_ee_phone_number
    deterministically_verify -> { Faker::PhoneNumber.phone_number[0..1] } do |phone_number_prefix|
      assert_include @valid_phone_prefixes, phone_number_prefix
    end
  end

  def test_ee_cell_phone
    deterministically_verify -> { Faker::PhoneNumber.cell_phone[0] } do |cell_phone_prefix|
